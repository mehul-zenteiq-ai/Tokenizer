...
# ══════════════════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
# 3.5 EXTRA CODE TOKENS (Forced BPE binding)
# ══════════════════════════════════════════════════════════════════════════════
EXTRA_CODE_TOKENS = [
    "==", "!=", ">=", "<=", "===", "!==", "<=>",
    "**", "//", "++", "--",
    ...
    "callable(", "enumerate(", "zip(", "map(", "filter(",
    "sorted(", "reversed(", "any(", "all(",
    "np.array(", "np.zeros(", "np.ones(", "np.sum(", "np.mean(",
    "np.dot(", "np.exp(", "np.log(", "np.sqrt(", "np.abs(",
    "np.reshape(", "np.concatenate(", "np.stack(", "np.squeeze(",
    "torch.tensor(", "torch.zeros(", "torch.ones(", "torch.cat(",
    "torch.stack(", "torch.matmul(", "torch.exp(", "torch.log(",
    "nn.Module", "nn.Linear(", "nn.Sequential(", "nn.Parameter(",
    "nn.Embedding(", "nn.LayerNorm(", "nn.Dropout(",
    "F.relu(", "F.softmax(", "F.cross_entropy(",
    "optimizer.zero_grad()", "optimizer.step()",
    "loss.backward()", ".detach()", ".requires_grad",
    "#include", "#define", "#ifndef", "#ifdef", "#endif",
    "#pragma", "#elif",
    "std::", "std::string", "std::vector", "std::cout", "std::endl",
    "std::cin", "std::map", "std::pair", "std::shared_ptr",
    "std::unique_ptr", "std::make_shared(", "std::make_unique(",
    "std::begin(", "std::end(", "std::move(", "std::forward(",
    "nullptr", "NULL",
    "printf(", "scanf(", "fprintf(", "sprintf(",
    "malloc(", "calloc(", "realloc(", "free(", "sizeof(",
    "template<", "vector<", "pair<",
    "Result<", "Option<", "Vec<",
    "Box<", "Arc<", "Mutex<", "RwLock<", "HashMap<",
    ...
    ".clone()", ".to_string()", ".as_str()", ".as_ref()",
    "console.log(", "console.error(", "console.warn(",
    "document.", "window.", "navigator.",
    "Promise.", "Promise.all(", "Promise.resolve(",
    ".then(", ".catch(", ".finally(",
    ...
    "@NotNull", "@Nullable",
    "ArrayList<", "List<", "Map<", "Set<",
    "Collections.", "Arrays.", "Objects.",
]
_N_CODE = len(EXTRA_CODE_TOKENS)
...
def _build_added_tokens():
    ...
    code_objs = [
        AddedToken(t, special=False, normalized=False,
                   single_word=False, lstrip=False, rstrip=False)
        for t in EXTRA_CODE_TOKENS]

    all_tokens = special_objs + sci_objs + ws_objs + code_objs
    seen, deduped = set(), []
...
def _patch_tokenizer_json(tok_path: Path) -> None:
    ...
    code_set = set(EXTRA_CODE_TOKENS)
    n_spec = n_sci = n_ws = n_code = 0
